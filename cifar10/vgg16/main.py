    ...
    if args.resume:
        ...
        if os.path.isfile(args.resume):
            ...
            if args.enable_bfa_mydefense:
                ...
                #checkpoint = torch.load('./save_finetune/cifar10_resnet32_quan_200_SGD_binarized/model_best.pth.tar')#checkpoint_branch.pth.tar#model_best_def.pth.tar
                #checkpoint = torch.load('/home/wangjialai/copy_for_use/flip_attack/BFA/save/cifar10_resnet20_quan_200_SGD_binarized/model_best.pth.tar')
                #checkpoint = torch.load('/home/wangjialai/copy_for_use/flip_attack/old_cifar/BFA_branch_2/save/cifar10_resnet20_quan_200_SGD_binarized/model_best_def.pth.tar')
            ...
            #tmp_stat = torch.load('/home/wangjialai/copy_for_use/flip_attack/BFA_sdn_new_branch/save_backup/cifar10_vgg16_quan_200_SGD_binarized/model_best.pth.tar')['state_dict']#checkpoint_branch.pth.tar#model_best_def.pth.tar
            
            # checkpoint_tmp = torch.load('/home/wangjialai/copy_for_use/flip_attack/BFA_sdn_branch/save_backup/cifar10_vgg16_quan_200_SGD_binarized/model_best.pth.tar')#checkpoint_branch.pth.tar#model_best_def.pth.tar
            # tmp_stat=checkpoint_tmp['state_dict']
            
            # for k,v in tmp_stat.items():
            #     if k in pretrained_dict:
            #         cha = v - pretrained_dict[k]
            #         print("cha:", k, cha[cha!=0])
            #sys.exit()
...
def perform_attack(attacker, model, model_clean, train_loader, test_loader,
    ...
    for i_iter in range(N_iter):
        ...
        if not random_attack:
            attack_log = attacker.progressive_bit_search(model, data, target, index_list[:])
        else:
...
def train(train_loader, model, criterion, optimizer, epoch, log, list_shape, flip_highest, num_branch, ic_only, net_flipped):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if ic_only:
            for idx in range(len(output_branch)):
                loss += criterion(output_branch[idx], target)
        else:
        ...
        if args.adv_train:#adv train = ROBUST IC
            ...
            if ic_only:
                for idx in range(len(flipped_out)):
                    ...
                    loss +=  criterion(flipped_out[idx], target)
            else:
...
def validate(val_loader, model, criterion, log, num_branch, ic_only, summary_output=False):
    ...
    if summary_output:
        ...
        if ic_only: #if only train ic branch
            for item in top1_list[:-1]:
                # print_log(f"item.avg: {item.avg}", log)
                sum += item.avg
            top1_avg = sum/len(top1_list[:-1])
            sum=0
